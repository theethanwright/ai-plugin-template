...
const Plugin: React.FC = () => {
  ...
  async function fetchFigmaData(fileKey: string) {
    try {
      console.log("Fetching data from Figma API with fileKey:", fileKey);
      const data = await figmaAPI.getFile(fileKey);
      console.log("Received data from Figma API:", data);
      setFigmaData(data);
    } catch (error) {
      console.error("Error fetching data from Figma API:", error);
      setError(error instanceof Error ? error.message : "An unknown error occurred while fetching Figma data");
    }
  }
  ...
  const handleFigmaFetch = async () => {
    const figmaFileKey = prompt("Enter the Figma file key:");
    if (figmaFileKey) {
      await fetchFigmaData(figmaFileKey);
    }
  };
